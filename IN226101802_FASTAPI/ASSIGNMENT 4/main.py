...
# --- TASK 1,3,4: ADD ITEMS TO CART + STOCK CHECK + DUPLICATE ITEMS ---

@app.post("/cart/add")
def add_to_cart(product_id: int, quantity: int = Query(..., gt=0)):
    # 1. Find the product in the master list
    product = next((p for p in products if p["id"] == product_id), None)
    
    # 2. Safety checks (from Task 3)
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
    if not product["in_stock"]:
        raise HTTPException(status_code=400, detail=f"{product['name']} is out of stock")

    # 3. NEW LOGIC: Check if this product is ALREADY in the cart
    for item in cart:
    ...
    # 4. If NOT in cart, add as a new item (original logic)
...
# --- TASK 5: DELETE FROM CART & CHECKOUT ---

# 1. DELETE /cart/{product_id} - Remove an item
...
def remove_from_cart(product_id: int):
    ...
    cart.remove(item)
    return {"message": f"Product '{item['product_name']}' removed from cart"}

# --- UPDATED TASK 6: CHECKOUT CREATES INDIVIDUAL ORDERS ---
...
def checkout(details: CheckoutRequest):
    ...
    # NEW LOGIC: Loop through each item and create a separate order entry
    for item in cart:
        order = {
    ...
    # Clear the cart after all orders are recorded
    cart.clear()
    
    return {
        "message": "Checkout successful", 
        "total_orders_created": len(orders_list)
    }

# 3. GET /orders - View all successful checkouts
@app.get("/orders")
def get_all_orders():
    return {"orders_placed": orders_list, "total_orders": len(orders_list)}
